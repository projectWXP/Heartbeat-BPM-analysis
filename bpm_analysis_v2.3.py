...
def plot_results(audio_envelope, peaks, smoothed_bpm, bpm_times, sample_rate, file_name):
    ...
    # annotations for Min, Max, and Average BPM
    if not smoothed_bpm.empty:
    ...
    # secondary x-axis for time in minutes
    max_time_sec = time_axis[-1] if len(time_axis) > 0 else 1
    tick_interval_sec = max(30, np.ceil(max_time_sec / 10)) # Aim for ~10 ticks
...
def analyze_wav_file(wav_file_path, params):
    ...
    audio_envelope, sample_rate = preprocess_audio(
        wav_file_path,
        params['downsample_factor'],
        params['bandpass_freqs'],
        params['save_debug_wav']
    )

    # 2. Find heartbeat peaks
    peaks = find_heartbeat_peaks(
        audio_envelope, sample_rate, params['min_bpm'], params['max_bpm'], params['s1_s2_max_interval_sec']
    )
