...
window_length = 5 # can adjust, higher = smoother graph, default 5
...
threshold_multiplier = 2.5 # can adjust, higher for higher threshold, default 2 to 3
...
segment_end = total_duration # End time in seconds, can adjust
...
high_res_downsample_factor = 10  # can adjust
downsample_factor = 100  # can adjust
audio_data_downsampled = audio_data_mono[segment_start_sample:segment_end_sample:downsample_factor]
time_downsampled = np.arange(segment_start_sample, segment_end_sample, downsample_factor) / sample_rate
audio_data_high_res_downsampled = audio_data_mono[segment_start_sample:segment_end_sample:high_res_downsample_factor]
...
fig.add_trace(go.Scatter(
    ...
    y=audio_data_high_res_downsampled,
    mode='lines',
...
for tick in xticks_positions:
    fig.add_annotation(
        ...
        showarrow=False,
        ax=0,
        ay=40 # can adjust position the annotations
    )
...
ax1.axhline(200, color='#e85858', linestyle='-', linewidth=1)
ax1.axhline(60, color='#8ede73', linestyle='-', linewidth=0.8)
ax1.axhline(max_bpm, color='#e36f6f', linestyle='--', linewidth=0.8)
ax1.axhline(min_bpm, color='#a3d194', linestyle='--', linewidth=0.8)
