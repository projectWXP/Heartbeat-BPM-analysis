...
def calculate_bpm_series(peaks, sample_rate, params):
    ...
    # Return the original numpy time points for compatibility with older functions
...
def plot_results(audio_envelope, peaks, all_raw_peaks, analysis_data, smoothed_bpm, bpm_times,
                 ...
                 major_inclines=None, major_declines=None, hrr_stats=None):
    ...
    max_time_sec = (time_axis_dt.max() - start_datetime).total_seconds() if len(time_axis_dt) > 0 else 1
    tick_vals_sec = np.arange(0, max_time_sec + 30, 30)
    tick_vals_dt = pd.to_datetime([start_datetime + datetime.timedelta(seconds=s) for s in tick_vals_sec])
    ...
    # --- Add HRR to the summary annotation ---
    if hrv_summary:
        ...
        if hrr_val is not None:
             annotation_text += f"<b>1-Min HRR: {hrr_val:.1f} BPM Drop</b><br>"
    ...
    # --- Visualize both inclines and declines ---
    if major_inclines:
        primary_incline = major_inclines[0]
        fig.add_shape(type="line",
            x0=primary_incline['start_time'], y0=primary_incline['start_bpm'],
            x1=primary_incline['end_time'], y1=primary_incline['end_bpm'],
            line=dict(color="purple", width=4, dash="dash"),
            xref='x', yref='y2'
        )
        fig.add_annotation(
            x=(primary_incline['start_time'] + primary_incline['end_time']) / 2,
            y=(primary_incline['start_bpm'] + primary_incline['end_bpm']) / 2 + 5,
            text=f"<b>Exertion Slope:<br>{primary_incline['slope_bpm_per_sec']:.2f} BPM/sec</b>",
            showarrow=False, font=dict(size=12, color="white"),
            bgcolor="purple", borderpad=4, xref='x', yref='y2'
        )

    if major_declines:
        primary_decline = major_declines[0]
        fig.add_shape(type="line",
            x0=primary_decline['start_time'], y0=primary_decline['start_bpm'],
            x1=primary_decline['end_time'], y1=primary_decline['end_bpm'],
            line=dict(color="#2ca02c", width=4, dash="dash"), # Green for recovery
            xref='x', yref='y2'
        )
        fig.add_annotation(
            x=(primary_decline['start_time'] + primary_decline['end_time']) / 2,
            y=(primary_decline['start_bpm'] + primary_decline['end_bpm']) / 2 - 5, # Position below the line
            text=f"<b>Recovery Slope:<br>{primary_decline['slope_bpm_per_sec']:.2f} BPM/sec</b>",
            showarrow=False, font=dict(size=12, color="white"),
            bgcolor="#2ca02c", borderpad=4, xref='x', yref='y2'
        )
    ...
    fig.update_layout(
        ...
        xaxis=dict(title_text="Time", tickvals=tick_vals_dt, ticktext=tick_text_combined),
        hovermode='x unified'
    )
...
def find_major_hr_inclines(smoothed_bpm_series, min_duration_sec=10, min_bpm_increase=15):
    ...
    # Find where the BPM is increasing
    is_increasing = smoothed_bpm_series.diff() > 0

    # Group consecutive periods of increase into distinct "events"
    # A new group starts every time the value changes from False to True
    incline_groups = (is_increasing != is_increasing.shift()).cumsum()

    major_inclines = []
    # Iterate through each potential incline event
    for group_id in incline_groups[is_increasing].unique():
        event = smoothed_bpm_series[incline_groups == group_id]

        if len(event) < 2:
            continue

        # Get start and end points of the event
        start_time, end_time = event.index.min(), event.index.max()
        start_bpm, end_bpm = event.iloc[0], event.iloc[-1]

        duration = (end_time - start_time).total_seconds()
        bpm_increase = end_bpm - start_bpm

        # Filter for significant events based on our thresholds
        if duration >= min_duration_sec and bpm_increase >= min_bpm_increase:
            # This is the direct slope calculation you wanted
            slope = bpm_increase / duration # Units: BPM per second
            ...
            major_inclines.append({
                ...
                'bpm_increase': bpm_increase,
                'slope_bpm_per_sec': slope
            })
            logging.info(f"Found significant incline: {duration:.1f}s duration, +{bpm_increase:.1f} BPM. Slope: {slope:.2f} BPM/s")

    # Sort by the steepest slope to have the most prominent one first
...
def find_major_hr_declines(smoothed_bpm_series, min_duration_sec=10, min_bpm_decrease=15):
    ...
    # Find where the BPM is decreasing
    is_decreasing = smoothed_bpm_series.diff() < 0

    # Group consecutive periods of decrease into distinct "events"
    decline_groups = (is_decreasing != is_decreasing.shift()).cumsum()

    major_declines = []
    for group_id in decline_groups[is_decreasing].unique():
        event = smoothed_bpm_series[decline_groups == group_id]
        ...
        if duration >= min_duration_sec and bpm_decrease >= min_bpm_decrease:
            ...
            major_declines.append({
                ...
                'end_bpm': end_bpm,
                'duration_sec': duration,
                'bpm_decrease': bpm_decrease,
                'slope_bpm_per_sec': slope
            })
            logging.info(f"Found significant decline: {duration:.1f}s duration, -{bpm_decrease:.1f} BPM. Slope: {slope:.2f} BPM/s")

    major_declines.sort(key=lambda x: x['slope_bpm_per_sec']) # Sort by steepest slope (most negative)
